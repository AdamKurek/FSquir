...
public sealed class RecordsDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            entity.ToTable("player_best_scores", table =>
            {
                table.HasCheckConstraint("ck_player_best_scores_coverage", "\"CoveragePercent\" >= 0 AND \"CoveragePercent\" <= 100");
            });
        ...
        {
            entity.ToTable("world_records", table =>
            {
                table.HasCheckConstraint("ck_world_records_coverage", "\"CoveragePercent\" >= 0 AND \"CoveragePercent\" <= 100");
            });
            entity.HasKey(x => x.Id);
            entity.Property(x => x.RulesVersion).IsRequired().HasMaxLength(32);
        ...
        {
            entity.ToTable("score_submission_logs", table =>
            {
                table.HasCheckConstraint("ck_score_submission_logs_coverage", "\"CoveragePercent\" >= 0 AND \"CoveragePercent\" <= 100");
            });
